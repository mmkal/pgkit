...
test(`statement to be ignored`, async () => {
  ...
  expect(logger.info).toHaveBeenCalledWith(
    './test/fixtures/ignore.test.ts/statement-to-be-ignored/ignore1.ts finished. Processed 0/1 queries (1 ignored).',
  )
  expect(logger.info).toHaveBeenCalledWith(
    './test/fixtures/ignore.test.ts/statement-to-be-ignored/process_normal.ts finished. Processed 1/1 queries.',
  )
  expect(logger.info).toHaveBeenCalledWith(
    './test/fixtures/ignore.test.ts/statement-to-be-ignored/mixed.ts finished. Processed 1/2 queries (1 ignored).',
  )
...
test(`fragment to be ignored`, async () => {
  ...
  expect(logger.info).toHaveBeenCalledWith('Initial codegen complete')
  expect(logger.warn).not.toHaveBeenCalled()
  expect(logger.error).not.toHaveBeenCalled()
