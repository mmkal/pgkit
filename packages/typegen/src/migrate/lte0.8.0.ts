...
export const migrate080 = async ({files, logger}: {files: string[]; logger: Logger}) => {
  ...
  for (const change of changes) {
    ...
    if (change.type === 'write') {
      ...
      fs.writeFileSync(change.file, await prettifyOne({filepath: change.file, content: change.content}))
...
const getChanges = (file: string): [FileChange] | [] => {
  ...
  const importSqlExpression = [`import { sql } from '@pgkit/client'`]

  const getImportSqlExpression = () => importSqlExpression.pop() || ''
  ...
  function visitNodes(node: ts.Node) {
    if (
      ...
      if (soloSqlImport || knownTypesImport || setupTypeGenImport) {
        edits.push({
          ...
          replacement: getImportSqlExpression(),
        })
      } else if (sqlImport) {
    ...
    if (
      ...
      node.tag.expression.getText() === 'sql'
    ) {
      edits.push({
    ...
    if (ts.isCallExpression(node) && node.expression.getText().endsWith('setupTypeGen')) {
