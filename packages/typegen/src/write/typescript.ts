...
const isValidIdentifier = (key: string) => !/\W/.test(key)

export const quotePropKey = (key: string) => (/\W/.test(key) ? JSON.stringify(key) : key)
...
export function renderQueryInterface(queryGroup: AnalysedQuery[], interfaceName: string) {
  ...
  const typeDef =
    interfaceName === '_void' && !/\w/g.test(bodies[0])
...
export function queryInterfaces(group: TaggedQuery[]) {
  ...
  uglyContent.unshift('// Generated by @pgkit/typegen')
