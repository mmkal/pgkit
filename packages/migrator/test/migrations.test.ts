...
import {SlonikMigrator} from '../dist'
  ...
  interceptors: [
    {
      afterPoolConnection: async (context, connection) => {
        ...
        // await connection.query(sql`set search_path to generated_new_schema`)
        return null
      },
    },
  ],
...
describe.skip('run migrations - neww', () => {
  const migrationsPath = join(__dirname, 'generated/run/migrations-new')
  ...
  test('up and down', async () => {
    const migrator = new SlonikMigrator({
      slonik: {
        ...
        transaction: handler =>
          slonik.transaction(async t => {
            await t.query(sql`set search_path foo`)
            return handler(t)
          }),
      },
      migrationsPath,
